...
## TASK
Write a {{ticker}} deep-dive voiceover of ~1,600–2,200 words (≈10–15 min at deadpan pace) with inline director tags, executing the chosen angle. Follow the FIXED SPINE — never a blank page:
1. COLD-OPEN REFRAME — the hook you chose below, one line that reframes why this left-for-dead name is worth fifteen minutes.
...
## RULES
- Multi-year first: growth rates, margin direction, share count, debt — the history table is the spine of section 3.
...
## OUTPUT
First the `=== HOOK OPTIONS ===` block, then the narration as plain text with inline tags — no JSON, no section headers, no stage directions other than the bracket tags. Begin the script at the chosen hook. Append the `=== ASSET PROMPTS ===` trailer ONLY if you used [ASSET] tags.
